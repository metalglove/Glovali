...
namespace Glovali.Common.Infrastructure.Networking
{
    ...
    public abstract class BaseNetworkConnector : INetworkConnector
    {
        ...
        private Task ProcessMessageTask(CancellationToken cancellationToken, string typeName, Memory<byte> bodyBufferMemory, byte[] bodyBufferSource)
        {
            ...
            {
                Debug.WriteLine("ProcessMessageTask started!");
                try
                {
                    Debug.WriteLine($"ProcessMessageTask: Message type {typeName}");
                    Debug.WriteLine($"ProcessMessageTask: Message body {Encoding.UTF8.GetString(bodyBufferMemory.Span)}");
                ...
                finally
                {
                    ...
                    Debug.WriteLine("ProcessMessageTask finished!");
                }
            }, cancellationToken);
        }
